...
console.log(splitMe("spero funzioni"));
/* ESERCIZIO 4
...
console.log(onlyLetters("ciao3  son4 le 5"));
/* ESERCIZIO 6
  Crea una funzione chiamata "isThisAnEmail" che riceve una stringa come parametro e ritorna true se la stringa è un valido indirizzo email.
*/
function isThisAnEmail(str) {
  let pattern = /[A-z0-9\.\+_-]+@[A-z0-9\._-]+\.[A-z]{2,6}/;
...
function isTodayMyBirthday() {
  ...
  if (birthdayDay == 1 && birthdayMonth == 2) {
    return true;
  }
  return false;
}
...
console.log(deleteProp(object, "prop"));
...
*/
function newwstMovie(movies) {
  let latestMovie = movies[0];
  movies.forEach((el) => {
    if (el.Year > latestMovie.Year) {
      latestMovie = el;
    }
  });
  return latestMovie;
}
console.log(newwstMovie(movies));
/* ESERCIZIO 13
...
function sumAllTheYears(movies) {
  return movies.reduce((acc, temp) => acc + temp.Year);
}
...
function searchAndDivide(movies, str) {
  ...
  foundFilms.match = movies.filter((el) => el.Title.includes(str));
  foundFilms.unmatch = movies.filter((el) => !el.Title.includes(str))
  return foundFilms;
}
...
*/
function stampaTdText(){
  let td = document.querySelectorAll("td")
  td.forEach((el) => console.log(el.innerHTML))
}
stampaTdText()
/* ESERCIZIO 23
